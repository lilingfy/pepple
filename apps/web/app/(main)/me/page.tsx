...
export default function MePage() {
  ...
  const [relationStats, setRelationStats] = useState<RelationStats | null>(null);
    ...
    <main className="container mx-auto px-6 py-8 max-w-5xl">
        ...
        <section className="bg-white/70 backdrop-blur-sm rounded-3xl p-6 shadow-sm border border-white/60">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center p-4 bg-[#A8D8B9]/10 rounded-2xl">
              <div className="text-3xl font-bold text-[#A8D8B9]">{relationStats?.count ?? 0}</div>
      ...
      <section className="bg-gradient-to-r from-[#A8D8B9]/20 to-[#7D8C9F]/20 rounded-3xl p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[#2C3E50] mb-1">管理你的人际关系图谱</h2>
            <p className="text-sm text-[#7D8C9F]">为不同关系对象建立档案，让 AI 分析更精准</p>
          </div>
          <Link
            href="/me/relations"
